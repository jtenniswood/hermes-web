...
test('hidden Bot visibility commands update the existing session-only preference', async ({ page }) => {
  ...
  const toggle = page.getByRole('menuitem', { name: 'Show hidden bots', exact: true })
  await filter.click()
  await expect(toggle.locator('.codicon-check')).toHaveCount(0)
  await toggle.click()
  await filter.click()
  await expect(toggle.locator('.codicon-check')).toHaveCount(1)
  await page.keyboard.press('Escape')
  await page.getByRole('tab', { name: 'Sessions', exact: true }).click()
  await page.getByRole('tab', { name: 'Bots', exact: true }).click()
  await filter.click()
