...
test('browser relocates the activity toggle without removing other Bots toolbar controls', () => {
  ...
  assert.match(output, /<Codicon name="add" size="0\.75rem" \/>/)
  assert.match(output, /<Codicon name="list-filter" size="0\.75rem" \/>/)
  assert.doesNotMatch(output, /'size-7 shrink-0 rounded-md text-\(--ui-text-tertiary\)/)
  assert.match(output, /<DropdownMenuSub>\s*<DropdownMenuSubTrigger>Show<\/DropdownMenuSubTrigger>/)
  assert.match(output, /<DropdownMenuSubTrigger>Filter by time<\/DropdownMenuSubTrigger>/)
  assert.match(output, /from '@\/components\/ui\/dropdown-menu'/)
  assert.ok(output.indexOf('<DropdownMenuSubTrigger>Show</DropdownMenuSubTrigger>') < output.indexOf('b.roster.botsOnly'))
  assert.ok(output.indexOf('<DropdownMenuSubTrigger>Filter by time</DropdownMenuSubTrigger>') < output.indexOf('b.roster.activeNow'))
  assert.ok(output.lastIndexOf('Show hidden bots') > output.lastIndexOf('b.roster.older'))
